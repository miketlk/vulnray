    ...
    if telemetry:
        lines.extend(
            [
                ...
                f"- Context expansions used: {telemetry.get('context_expansions_used', 0)}",
                f"- Symbols added to context: {telemetry.get('symbols_added_to_context', 0)}",
                f"- Types added to context: {telemetry.get('types_added_to_context', 0)}",
                f"- Caller summaries added: {telemetry.get('caller_summaries_added', 0)}",
                f"- Detection parse failures: {telemetry.get('detection_parse_failures', 0)}",
